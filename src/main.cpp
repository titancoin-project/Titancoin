...
uint256 hashGenesisBlock("0xd80dad28e1eca34b7e63d4a0e945eb174e5ff5f16481ac645326181e15de9b34");
...
{
    // Titancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1M blocks, which will occur approximately every 3 years
    nSubsidy >>= (nHeight / 1000000); // Titancoin: 1M blocks in ~3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.1 * 24 * 60 * 60; // Titancoin: 2.1 days
static const int64 nTargetSpacing = 90; // Titancoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Titancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Titancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd80dad28e1eca34b7e63d4a0e945eb174e5ff5f16481ac645326181e15de9b34");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "30/06/2017 Titan's coin creation day";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041871710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498843664;
        ...
        assert(block.hashMerkleRoot == uint256("0xa4a67c7dacfbdd9f9c1ed3bda346ccb17f240e45729fe0a4738581c0fe7dc4e3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc2, 0xd3, 0xd1, 0xbd }; // Titancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TitancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TitancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TitancoinMiner(CWallet *pwallet)
{
    printf("TitancoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running TitancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TitancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TitancoinMiner, pwallet));
}
